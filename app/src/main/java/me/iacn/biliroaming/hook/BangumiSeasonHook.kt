...
class BangumiSeasonHook(classLoader: ClassLoader) : BaseHook(classLoader) {
    ...
    override fun startHook() {
        ...
        val viewUniteReqClass = "com.bapis.bilibili.app.viewunite.v1.ViewReq".from(mClassLoader)
        val viewUniteReplyClass = "com.bapis.bilibili.app.viewunite.v1.ViewReply".from(mClassLoader)
        if (viewUniteReqClass != null && viewUniteReplyClass != null) {
            instance.viewUniteMossClass?.hookAfterMethod(
                ...
                if (response == null) {
                    ...
                    val av =
                        (if (req.hasAid()) req.aid.takeIf { it != 0L } else if (req.hasBvid()) bv2av(req.bvid) else null)?.toString()
                    fixViewProto(req, av)?.toByteArray()?.let {
                        param.result = viewUniteReplyClass.callStaticMethod("parseFrom", it)
